...
app.config['SQLALCHEMY_DATABASE_URI']='postgresql://postgres:postgres@localhost:5432/fyur'
db = SQLAlchemy(app)
...
def show_venue(venue_id):
  ...
  upcomingshows = db.session.query(Show, Artist).join(Artist).join(Venue).filter(Show.venue_id==venue_id).filter(Show.start_time>datetime.now()).all()
  upcoming_shows =[]
...
def show_artist(artist_id):
  ...
  upcomingshows = db.session.query(Show, Venue).join(Artist).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time>datetime.now()).all()
  upcoming_shows =[]
  for show in upcomingshows:
    upcoming_shows.append({
      "artist_id": show.Show.artist_id,
...
def create_show_submission():
  ...
  # see: http://flask.pocoo.org/docs/1.0/patterns/flashing/  except:
  except:
    error = True
    db.session.rollback()
    print(sys.exc_info())
